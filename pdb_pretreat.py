...
from SSPPIopts import radii, polarHydrogens
import random
import pymesh
import time
...
def computeMSMS(pdb_file):
    ...
    file_base = SSPPIopts['tmp_dir']+"/msms_"+str(randnum)
...
in_fields = sys.argv[1].split('/')[-1].split(".")[0].split('_')
pdb_id = in_fields[0]
chain_ids1 = in_fields[1]

pdb_filename = SSPPIopts['raw_pdb_dir']+pdb_id+".pdb"
tmp_dir= SSPPIopts['tmp_dir']
protonated_file = tmp_dir+"/"+pdb_id+".pdb"
print(pdb_filename)
print(tmp_dir)
print(protonated_file)
...
for i in range(len(geometry_sum)):
    for j in geometry_sum[i][1]:
        if "_" in str(j):
            fp1.write(str(j).split("_")[1]+"_"+str(codes[str(j).split("_")[3]])+"\n")
        else: 
            fp1.write(str(j)+"\t")


if not os.path.exists(SSPPIopts['ply_chain_dir']):
    os.makedirs(SSPPIopts['ply_chain_dir'])
if not os.path.exists(SSPPIopts['pdb_chain_dir']):
    os.makedirs(SSPPIopts['pdb_chain_dir'])
shutil.copy(out_filename1+'.ply', SSPPIopts['ply_chain_dir'])
shutil.copy(out_filename1+'.pdb', SSPPIopts['pdb_chain_dir'])
